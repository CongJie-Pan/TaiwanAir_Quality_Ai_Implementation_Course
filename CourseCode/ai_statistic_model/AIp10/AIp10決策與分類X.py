# AIp10決策與分類X.py: AI python 實作 - 10: 決策與分類
# Jia-Sheng Heh, 11/12/2024, revised from AIp09時序與回歸.py
# Usage: streamlit run AIp10決策與分類X.py --> http:/localhost:8501
...
font_path = os.path.join(wkDir, '微軟正黑體-1.ttf')   #-- # 加載微軟正黑體字型 (請將字型檔放置於腳本同目錄)
...
def graph_style(rules):              ##== 從關聯規則(rules,及選取數據): graph, style = graph_style(rules,XX)
    ...
    lenXX = rules["count"].iloc[0] / rules["antecedent support"].iloc[0]  # rules["count"][0]/rules["antecedent support"][0]
...
def plot_decomposition(decomposition):   
    fig = make_subplots(rows=4, cols=1, shared_xaxes=True, vertical_spacing=0.05, 
                        subplot_titles=("原始值(Observed)", "趨勢(Trend)", "季節性(Seasonal)", "殘差(Residual)"))
    fig.add_trace(go.Scatter(x=decomposition.observed.index, y=decomposition.observed, mode='markers+lines', name='原始值(Observed)'), row=1, col=1)
    fig.add_trace(go.Scatter(x=decomposition.trend.index,    y=decomposition.trend,    mode='markers+lines', name='趨勢(Trend)'),      row=2, col=1)
    fig.add_trace(go.Scatter(x=decomposition.seasonal.index, y=decomposition.seasonal, mode='markers+lines', name='季節性(Seasonal)'), row=3, col=1)
    fig.add_trace(go.Scatter(x=decomposition.resid.index,    y=decomposition.resid,    mode='markers+lines', name='殘差(Residual)'),   row=4, col=1)
    fig.update_layout(title="時序分解 (Time Series Decomposition)", height=800, showlegend=False)  
    return fig
...
def prepDTdata(XX):             ##== 準備決策樹數據 ==##
    ...
    XX1 = XX[["channel","category","category2","price0"]];   yy1 = XX["quarter"];    print(XX1[17670:17674])
    return XX1,yy1
...
def plotONCtree(tree_modelH, encoder, yy1H_encoder):   ##== 可視化決策樹 (tree_modelH) ==##
    ...
    fig = plot_tree(tree_modelH, feature_names=encoder.get_feature_names_out(), class_names=yy1H_encoder.classes_, 
              filled=True, fontsize=8);   plt.title("以 ONE-HOT_encoding 求得的決策樹")
    plt.show()
    return fig
...
def 擷取交易(Xname):      ##== (KDD1) X, rMsg, log = 擷取交易(Xname) ==##
    ...
    return X, rMsg, log
def 營業額趨勢預測(X):    ##== (KDD2) X = 營業額趨勢(X) ==##
    ##== (d).後台 ==##   #==> [[AIp04/C3)(1)後台]]
    Sv = X.groupby(["ym"]).agg({"amount":"sum"}).reset_index();  print(Sv.shape);   print(Sv.head(2))   #-- (36, 2)
    #            ym   amount
    # 0  2015-01-01  2164967
    # 1  2015-02-01  2708844
    min_date = Sv['ym'].min().date();    max_date = Sv['ym'].max().date()
    fig1 = px.line(Sv, x="ym", y="amount", text="amount", title="整體營業額");   # fig.show()
    fig1 = go.Figure(go.Scatter( x = Sv['ym'], y = Sv['amount'], ))
    fig1.update_traces(mode='markers+lines');   
    ##== (c).前台-canvas ==##
    st.header("== (1).(KDD2) 營業額概況 --");         #-- (d).前台--canvas
    st.subheader("* 交易區間: "+ str(X["date"].min()) + " -- " + str(X["date"].max()))        
    st.subheader("* 共有"+ str(X.shape[0])+ "筆交易記錄")        
    st.subheader("* 總營業額為"+ str(np.sum(X["amount"])) + "元")
    st.header("== (2).(KDD2) 營業額趨勢 --");  cols = st.columns([1,2])  #-- (d).前台--canvas
    cols[0].subheader("* (A) 各月營業額");       
    cols[0].write("* 共有"+ str(np.sum(Sv["amount"]))+ "元");   cols[0].dataframe(Sv)
    cols[1].subheader("* (B) 營業額趨勢");     cols[1].plotly_chart(fig1, theme="streamlit", use_container_width=True)
    ##== (b).前台-sidebar ==##  
    st.sidebar.header("== (1) (KDD2).營業額概況 --"); #-- (b).前台--sidebar
    st.sidebar.write("* 交易區間: "+ str(X["date"].min()) + " -- " + str(X["date"].max()))        
    st.sidebar.write("* 共有"+ str(X.shape[0])+ "筆交易記錄")                         
    st.sidebar.header("== (3) (KDD2).選取營業額區間 --"); #-- (b).前台--sidebar
    selected_date = st.sidebar.slider("> 選擇日期上限", min_value=min_date, max_value=max_date, value=max_date, format="YYYY-MM")
    ##== (d).後台 ==##   
    data = Sv[Sv['ym'] <= pd.to_datetime(selected_date)]
    duration = "交易區間: " + data["ym"].min().strftime("%Y/%m") + "--" + data["ym"].max().strftime("%Y/%m")
    decomposition = seasonal_decompose(data["amount"], model="additive", period=7)
    fig3 = plot_decomposition(decomposition)   # fig3 = decomposition.plot()
    fig2 = px.line(data, x="ym", y="amount", text="amount", title="整體營業額");   # fig.show()
    fig2 = go.Figure(go.Scatter( x = data['ym'], y = data['amount'], ))
    fig2.update_traces(mode='markers+lines');   
    data.set_index('ym', inplace=True)  # 設置日期為索引
    data.index = pd.to_datetime(data.index)     
    ##== (c).前台-canvas ==##
    st.header("== (3) (KDD4).區間營業額:"+duration);  cols = st.columns([1,2])  #-- (d).前台--canvas
    cols[0].subheader("* (A) 區間月營業額趨勢");       
    cols[0].write("* 共有"+ str(np.sum(data["amount"]))+ "元")        
    cols[0].plotly_chart(fig2, theme="streamlit", use_container_width=True)
    cols[1].subheader("* (B) 區間營業額分解 = 趨勢(Trend) + 季節性(Seasonal) + 殘差(Residual)");       cols[1].plotly_chart(fig3, theme="streamlit", use_container_width=True) # cols[1].st.pyplot(fig3)
    ##== (b).前台-sidebar ==##  
    st.sidebar.header("== (4) (KDD4).區間營業額預估 --")
    if st.sidebar.checkbox("> 開始進行預估 --"): #-- (b).前台--sidebar
        st.header("== (4) (KDD4).預估營業額 --");       
        #-- HW 訓練與繪圖例
        ...
        fig_hw = plot_predictions(combined_prediction_hw, title="Holt-Winter Forecast")
        st.subheader("* (A) Holt-Winter模型 --");   cols = st.columns([1,2])  #-- (d).前台--canvas
        cols[0].write(hw_model.params)    
        cols[1].plotly_chart(fig_hw, theme="streamlit", use_container_width=True)
        #-- ARIMA 訓練與繪圖例
        ...
        st.subheader("* (B) ARIMA模型 --");   cols = st.columns([1,2])  #-- (d).前台--canvas
        cols[0].write(arima_model.params)    
        cols[1].plotly_chart(fig_arima, theme="streamlit", use_container_width=True)
        #-- SARIMA 訓練與繪圖例
        ...
        st.subheader("* (C) SARIMA模型 --");   cols = st.columns([1,2])  #-- (d).前台--canvas
        cols[0].write(sarima_model.params)    
        cols[1].plotly_chart(fig_sarima, theme="streamlit", use_container_width=True)
    return X, [], []
...
def 設定TA交易(CvTA,X): ##== (KDD3,4) XX, rMsg, log = 設定TA交易(CvTA,X) ==##
    ##== (a).後台 ==##
...
def 客群關連規則(XX):     ##== (KDD3,4) XX, rMsg, log = 客群關連規則(XX) ==##
    ...
    log = [ ]
    return XX, rMsg, log
def 購買季節決策(XX):     ##== (KDD3,4) XX, rMsg, log = 購買季節決策(XX) ==##
    from sklearn.tree import plot_tree
    import matplotlib.font_manager as fm
    plt.rcParams['font.family'] = ['SimHei']  # 例如：SimHei 黑體
    plt.rcParams['axes.unicode_minus'] = False  # 解決負號 '-' 顯示問題
    ##== (a).後台 ==##
    XX["yq"] = pd.PeriodIndex(XX.date, freq='Q')
    XX["ym"] = pd.PeriodIndex(XX.date, freq='M')
    XX1,yy1 = prepDTdata(XX)
    ##== (c).前台-canvas ==##
    st.header("== (KDD3-4).購買季節決策(XX) -- ")     
    st.subheader("(1).(KDD3) 目標客群之交易 (XX) -- ")
    st.write(f"* 目標客群交易 XX 具有 {XX.shape[0]} 筆記錄")
    cols = st.columns([1,1])
    cols[0].subheader("(2A).(KDD3) 每位客戶交易的自變量 (XX1) -- ");   cols[0].dataframe(XX1.head(5))
    cols[1].subheader("(2B).(KDD3) 每位客戶交易的因變量 (yy1) -- ");   cols[1].dataframe(yy1[0:5])
    ##== (d).前台-sidebar ==##  #==> [[AIp04/C4)(2)垂直流程]]
    st.sidebar.header( "== (1-2).(KDD3) 購買季節決策之數據準備(XX) -- " )
    st.sidebar.write(f"* 目標客群有 {XX.shape[0]} 筆交易")
    st.sidebar.subheader("== (3).(KDD4) 購買季節決策(XX) -- ")     
    nDepth = st.sidebar.slider('- 決策樹層次 = ', min_value=2, max_value=5, value=3, step=1)
    if st.sidebar.checkbox(f"決策樹層級 nDepth={nDepth}"):                 
        DTmodel, DTrules, encoder, yy1H_encoder = ONCtree(XX1,yy1,nDepth);   print(DTrules)
        cols = st.columns([1,3])
        cols[0].subheader("- (4A) 決策規則:");  cols[0].text(DTrules)                
        cols[1].subheader("- (4B) 決策樹圖:");  
        fig, ax = plt.subplots(figsize=(12, 8))
        plot_tree(DTmodel, feature_names=encoder.get_feature_names_out(), 
                  class_names=yy1H_encoder.classes_, filled=True, ax=ax, fontsize=6)
        plt.title("決策樹視覺化")
        cols[1].pyplot(fig)
    ##== (e/f).返回信息(rMsg)/LOG(log) ==##
    rMsg = [ ]
...
def check2log(strList,log,rv_list):   ##== check 再將 textStr 納入 log 中, 並中並可以提供建議, 並生成投影片 ==##
    ...
    with st.sidebar.expander("___ 用戶可以在下方直接輸入[用戶名]與[建議]進行鹬讀 .....", expanded=True): 
        ##-- 生成建議 --## 
        st.session_state.username = st.text_input("用戶名", st.session_state.username, placeholder="輸入用戶名", label_visibility="collapsed")
        sugg_key = f"log_checkbox_{len(log)}"
        suggestion = st.text_area("建議", value=st.session_state.suggestion, key=sugg_key, height=20, placeholder="輸入建議", label_visibility="collapsed")
...
if __name__ == "__main__":
    ...
    st.set_page_config(page_title="AIp10決策與分類(C01D)", page_icon="✅", layout="wide",)  #==> [[AIp04/C4)(5)加上頁註,頁標題等]]
    page = st_navbar(["[1擷取交易]", "[1A營業額趨勢預測]", "[2轉換客戶圖像]", "[3TFM_客群選取]", "[4設定TA交易]", "[5客群聚類分析]", "[6客群關連規則]", "[7購買季節決策]", "[8匯出PPT檔]", "(C)"])
    ...
    st.title("AIp10決策與分類(C01D)儀表板")
    st.sidebar.title("控制盤(C01D)--")
    ...
    match page:
        ...
        case "[1A營業額趨勢預測]": sss.X, rv_list, log_list = 營業額趨勢預測(sss.X);    check2log(log_list, sss.LOG, rv_list)
        case "[2轉換客戶圖像]":
        ...
        case "[4設定TA交易]":
            ...
            else: sss.XX, rv_list, log_list = 設定TA交易(sss.CvTAtfm, sss.X);     check2log(log_list, sss.LOG, rv_list)
        case "[5客群聚類分析]":
        ...
        case "[7購買季節決策]":
            if sss.XX is None: st.write("尚未選取目標客群交易，請先選取目標客群！");   sss.LOG.append("尚未選取目標客群交易，請先選取目標客群")
            else: sss.CP_matrix, rv_list, log_list = 購買季節決策(sss.XX);     check2log(log_list, sss.LOG, rv_list)
        case "[8匯出PPT檔]":
            if sss.Cv is None: st.write("尚未轉換客戶圖像，請先計算客戶圖像！");   sss.LOG.append("尚未轉換客戶圖像，請先轉換客戶圖像！")
