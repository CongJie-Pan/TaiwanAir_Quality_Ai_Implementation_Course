# AIp07聚類及評估X.py: AI python 實作 - 07: 聚類及評估
# Jia-Sheng Heh, 10/22/2024, revised from jjsAIDA架構07.py
# Usage: streamlit run AIp07聚類及評估X.py --> http:/localhost:8501
...
def 目標客群交易(CvTA,X): ##== (KDD3,4) XX, rMsg, log = 目標客群交易(CvTA,X) ==##
    ...
    return XX, rMsg, log
def 客戶品類聚類(XX): ##== (KDD3,4) XX, rMsg, log = 目標客群交易(CvTA,X) ==##
    import matplotlib.pyplot as plt
    from sklearn.preprocessing import StandardScaler
    from scipy.cluster.hierarchy import dendrogram, linkage, fcluster
    import matplotlib.font_manager as fm
    ...
    if st.sidebar.checkbox("繪製聚類結果樹"):        
        ...
        cols = st.columns([1, 1])    #== 營業額模型 (TFMvs,TFMv0s)
...
if __name__ == "__main__":
    ...
    st.set_page_config(page_title="AIp07聚類數據規畫(C01A)", page_icon="✅", layout="wide",)  #==> [[AIp04/C4)(5)加上頁註,頁標題等]]
    page = st_navbar(["[1擷取交易]", "[2轉換客戶圖像]", "[3TFM_客群選取]", "[4目標客群交易]", "[5客戶品類聚類]", "[6匯出PPT檔]", "(C)"])
    ...
    st.title("AIp07聚類數據規畫(C01A)儀表板")
    st.sidebar.title("控制盤(C01A)--")
    ...
    match page:
        ...
        case "[2轉換客戶圖像]":
            ...
            # st.subheader("test:");   st.write(sss.Cv.head(3))
        case "[3TFM_客群選取]":
        ...
        case "[5客戶品類聚類]":
            ...
            else: sss.CP_matrix, rv_list, log_list = 客戶品類聚類(sss.XX);     check2log(log_list, sss.LOG, rv_list)
        case "[6匯出PPT檔]":
            if sss.Cv is None: st.write("尚未轉換客戶圖像，請先計算客戶圖像！");   sss.LOG.append("尚未轉換客戶圖像，請先轉換客戶圖像！")
