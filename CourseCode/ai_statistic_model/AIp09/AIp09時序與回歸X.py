# AIp09時序與回歸X.py: AI python 實作 - 09: 時序與回歸
# Jia-Sheng Heh, 11/10/2024, revised from AIp08關連的挖掘.py
# Usage: streamlit run AIp09時序與回歸X.py --> http:/localhost:8501
...
if __name__ == "__main__":
    ...
    st.set_page_config(page_title="AIp09時序回歸分析(C01C)", page_icon="✅", layout="wide",)  #==> [[AIp04/C4)(5)加上頁註,頁標題等]]
    page = st_navbar(["[1擷取交易]", "[1A營業額趨勢預測]", "[2轉換客戶圖像]", "[3TFM_客群選取]", "[4目標客群交易]", "[5客群聚類分析]", "[6客群關連規則]", "[7匯出PPT檔]", "(C)"])
    ...
    st.title("AIp09時序回歸分析(C01C)儀表板")
    st.sidebar.title("控制盤(C01C)--")
